...
public:
	...
	*/
	...
	//! \brief Retrieves the path and name of the source file.
	/*! The source file of an xref record is the file from where the source scene entities 
	xrefed via this xref record come from.
	\return - Pointer to the path and name of the file that the source scene entity comes 
	from or or NULL if there's no source file specified.
	*/
	virtual const TCHAR* GetSrcFileName() const = 0;
	//@}
	...
	\return - true if the operation succeeded, orthewise false.
	*/
	...
	*/
	...
	BEGIN_FUNCTION_MAP
		...
		RO_PROP_FN(kfpGetSrcFileName, GetSrcFileName, TYPE_FILENAME);
...
inline unsigned int IObjXRefRecord::FPItemCount(IntTab& itemTypes) const
{
	// convert enum tab array to enum
	unsigned int l_types = 0;
	for (int i = 0; i < itemTypes.Count(); i++)
		l_types |= itemTypes[i];
	return ItemCount(static_cast<IXRefItem::XRefItemType>(l_types));
}

inline ReferenceTarget* IObjXRefRecord::FPGetItem(size_t i, unsigned int itemType) const
{
	return GetItem(i, static_cast<IXRefItem::XRefItemType>(itemType));
}

inline unsigned int IObjXRefRecord::FPGetItems(IntTab& itemTypes, Tab<ReferenceTarget*>& xrefItems) const
{
	// convert enum tab array to enum
	unsigned int l_types = 0;
	for (int i = 0; i < itemTypes.Count(); i++)
		l_types |= itemTypes[i];
	return GetItems(static_cast<IXRefItem::XRefItemType>(l_types), xrefItems);
}

inline IntTab IObjXRefRecord::FPGetXRefOptions() const
{
	unsigned int l_myOptions = GetXRefOptions();
	// convert enum to enum tab array
	IntTab l_optionTab;
	l_optionTab.SetCount(0);
	DWORD bit = 1;
	for (int i = 0; i < sizeof(DWORD); i++)
	{
		if (l_myOptions & bit) 
			l_optionTab.Append(1,(int*)&bit);
		bit = bit << 1;
	}
	return l_optionTab;
}
