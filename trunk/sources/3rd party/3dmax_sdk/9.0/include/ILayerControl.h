...
public:
	...
	//! then nothing occurs.
	//! \param[in] iundex The index to set active. 
	...
	//! \param[i] nodeTab The nodes where the layer will be copy.  Nothing happens if the layer doesn't exist on a node.
	...
	//! \param[in] time At what time to get the layer weight.
	...
	//! \param[in] time At what time to set the layer weight.
...
//! \brief Get the LayerControl Interface \see ILayerControl
