...
void Array<T>::quickSortRecursive(T* data, size_t first, size_t last, CompareFnc cmp) {
	...
	if(first < last) {
		const T& pivot = data[first];
		size_t left = first + 1;
		size_t right = last;

		while(left <= right) {
			while((left <= right) && (cmp(&(data[left]), &pivot) <= 0)) {
				++left;
			}
			while((left <= right) && (cmp(&(data[right]), &pivot) > 0)) {
				--right;
			}
			if(left < right) {
		...
		// Move the pivot
		{
			T swapValue = data[first];
			data[first] = data[right];
			data[right] = swapValue;
		}

		quickSortRecursive(data, first, right - 1, cmp);
		quickSortRecursive(data, right + 1, last, cmp);
	}
}
