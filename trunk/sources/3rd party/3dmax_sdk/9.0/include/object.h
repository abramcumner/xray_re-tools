...
/*! \defgroup Scene and Node Hit Testing Flags
The following represent flags describing various hit testing flags that can be sent to the node and 
scene hit testing methods
*/
//@{
#define HIT_SELONLY				(1<<0)  //!< Hit test selected items only.
#define HIT_UNSELONLY			(1<<2)  //!< Hit test unselected items only.
#define HIT_ABORTONHIT			(1<<3)  //!< Abort the process of hit testing after finding any hit.
#define HIT_SELSOLID			(1<<4)  //!< This treats selected items as solid and unselected items as not solid. Treating an item as solid means the face will be hit if the mouse is anywhere inside the face region and not just over a visible edge.
#define HIT_ANYSOLID			(1<<5)  //!< This treats any item as solid.
#define HIT_TRANSFORMGIZMO		(1<<6)	//!< this forces hit testing for the transform gizmos
#define HIT_SWITCH_GIZMO		(1<<7)	//!< this forces hit testing for the Switch axis when hit
#define HIT_MANIP_SUBHIT        (1<<8)  //!< this forces hit testing for sub-manipulators


#define HITFLTR_ALL             (1<<10) //!< This is filters for hit testing everything which can be combined into the flags parameter.
#define HITFLTR_OBJECTS         (1<<11) //!< This is filters for hit testing just objects which can be combined into the flags parameter.
#define HITFLTR_CAMERAS         (1<<12) //!< This is filters for hit testing just cameras which can be combined into the flags parameter.
#define HITFLTR_LIGHTS          (1<<13) //!< This is filters for hit testing just lights which can be combined into the flags parameter.
#define HITFLTR_HELPERS         (1<<14) //!< This is filters for hit testing just helpers which can be combined into the flags parameter.
#define HITFLTR_WSMOBJECTS      (1<<15) //!< This is filters for hit testing just world space objects which can be combined into the flags parameter.
#define HITFLTR_SPLINES         (1<<16) //!< This is filters for hit testing just splines which can be combined into the flags parameter.
#define HITFLTR_BONES	        (1<<17) //!< This is filters for hit testing just bones which can be combined into the flags parameter.
#define HIT_SCENEXREFS	        (1<<18) //!< This lets you hitest scene xrefx however DO NOT make references to them 
#define HITFLAG_STARTUSERBIT    24      //!< Starting at this bit through the 31st bit can be used by plug-ins for sub-object hit testing
//@}
...
public:     
	...
	*/
	...
	/*! \param [in] node - The node to add to the array
	\param [in] allowDups - If true, the specified node is added to the array 
	...
	\param [in] allocExtra - The number of extra items by which the array should 
	...
	*/
	...
	/*! \param [in] node - The node to add to the array
	\param [in] at - Array index where to insert the specified node. If a negative 
...
public: 
	//! Constructor
	//! \param[in] so - ShapeObject that publishes this interface
	IShapeRectRenderParams(ShapeObject *so) : mShape(so)
	{
	}

	//! Gets the Rectangular setting of the shape
	//! \param[in] t - Time to get the value for
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] rectangular - if true, shape is rectangular, if false radial
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] width - Width Value (must be positive)
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] length - Length Value (must be positive)
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] angle - Angle Value for rectangular shape
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] aspectLock - TRUE if aspect is locked, FALSE otherwise
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] autosmooth - TRUE if resulting mesh should be autosmoothed, FALSE otherwise
	...
	//! \param[in] t - Time to set the value for
	//! \param[in] threshold - threashold angle in radians
...
public:
	...
	\param[in] id An id used to identify the property.
	\param[in] data A pointer (needs to be castable to AnimProperty *) that contains the 
	data to be kept.
	\return returns 1
	*/
	...
	\param[in] id Id that is associated with desired data.
...
private:
	...
	\param[in] hWnd Handle to spinner whose aspect has changed.
	*/
	void SetAspect(HWND hWnd);
	/*!Calculates aspect ratio of viewport or renderer settings.
	\param[in] t time value which calculation is made at
	\param[in] viewport control parameter to indicate whether viewport or renderer is desired
