...
public:
	...
	CoreExport void   Free() { 
      if( handle ) { 
         FreeLibrary(handle); handle = NULL; 
			vLibShutdown = NULL; vLibInitialize = NULL;
			loaded = false; 
      } 
   }
   ...
   // New with Max9; these are optional and can be NULL.
   BOOL           (*vLibInitialize)(); // Initialize the DLL, if it needs to do more involved
                                       // initializations that should not be done from
                                       // within DllMain().  (See Microsoft's documentation
                                       // on DllMain()).  If present this is the first
                                       // function called in a DLL; it may be called
                                       // more than once, so the function should be prepared
                                       // to handle it.  We try to call it just once though.
   BOOL           (*vLibShutdown)();   // Inversly, performs cleanup before the DLL is
                                       // unloaded.
