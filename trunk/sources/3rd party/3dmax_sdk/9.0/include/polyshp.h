...
class PolyLine: public MaxHeapOperators {
	public:
		...
		/*!	This renders the polyline to the display.
				\param[in] GraphicsWindow *gw Points to the graphics window to render to.
				\param[in] Material *ma The list of materials to use to render the polyline. See Class Material, Class INode - Material methods.
				\param[in] RECT *rp Specifies the rectangular region to render. If the polyline should be rendered to the entire viewport pass NULL.
				\param[in] int numMat The number of materials for the polyline.
				\param[in] BOOL colorSegs if this is set material is used to determine what colors to render the polyline. Otherwise the standard sel/subsel colors are used
				\param[in] BitArray segsel this is the segment sel bitarray used to draw the selected segments
				\param[in] BOOL useStartSegments = TRUE if this is set the start/endsegmens will be used around the polyline draw.  If you are drawing several polylines in a row it is faster to set this to false and use start/endsegments around the entire draw instead of each individual polyline
		*/
