	...
	if (not((::recompile()) or (::level_graph()) or (::add_edge()) or  (::ignore()) or ($self->{build_version} eq '1469' or $self->{build_version} eq '1472' or $self->{build_version} eq '1510'))) {
		...
		foreach my $vertex (@{$self->{vertices}}) {
			...
			if (!defined $self->{lp_offsets}{$$level->{level_name}}) {
				$self->{lp_offsets}{$$level->{level_name}} = $vertex->{level_point_offset};    ####оффсеты идут от блока левелов!
	...
	if ($self->{build_version} eq 'cop') {
		if (::recompile()) {
			...
			foreach my $level (@{$self->{levels}}) {
				...
				$self->{ct_offset}{$level->{level_name}} = $initial_offset;   	####оффсеты идут с начала файла!
...
##### Раз чтение кросс-таблиц и левел-поинтов жрет много памяти, будем доставать нужные данные прямо перед записью. ####	
