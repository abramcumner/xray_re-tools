...
#	{ name => 'unused',		type => 'h16',	default => 0 },	# 0x30
#	{ name => 'spawn_id',	type => 'h16',	default => 0xffff},
#	{ name => 'extended_size',	type => 'u16',	default => 0x31 },	# 0x32
);
sub state_read {
	my $self = shift;
	my ($packet) = @_;
...
sub state_write {
	if (::split_spawns()) {
		$_[0]->{game_vertex_id} = 0xffff;
		$_[0]->{level_vertex_id} = 0xffffffff;
	}
	$_[1]->pack_properties($_[0], properties_info);
}
sub state_import {
...
sub read {
	$_[1]->unpack_properties($_[0], header);		
}
#######################################################################
package graph_level;
use strict;
...
sub read {
	$_[1]->unpack_properties($_[0], level);		
}
#######################################################################
package graph_vertex;
use strict;
use constant vertex => (
...
sub read {
	$_[1]->unpack_properties($_[0], vertex);
	$_[0]->{edge_index} = ($_[0]->{edge_offset} - $_[2]) / 0x6;
	$_[0]->{game_vertex_id} = $_[3];
	my $level_name = levels::name_by_gvid($_[0]->{game_vertex_id});;
	$_[0]->{name} = join('_', $level_name,'graph_point',$_[4]);
#	print "$_[0]->{name}\n";
}
#######################################################################
package graph_edge;
...
sub read {
	$_[1]->unpack_properties($_[0], edge);
}
#######################################################################
...
use constant section_to_class => {
	...
	####
	item_key			=> 'cse_alife_item',
	transiver_1			=> 'cse_alife_item',
	scanner_device			=> 'cse_alife_item',
...
$packet->length() == 0 or print "data left [".$packet->length()."] in $self->{cse_object}->{name}\n";
...
$packet->length() == 0 or print "update data left [".$packet->length()."] in $self->{cse_object}->{name}\n";
				}
				$cf->r_chunk_close();
			}
		}
...
use constant levels_info => (
	{ gvid0 => 934, },
#	{ gvid0 => 3798,		name => 'l10u_bunker' },
#	{ gvid0 => 3748,		name => 'l12u_control_monolith' },
#	{ gvid0 => 3683,		name => 'l12u_sarcofag' },
#	{ gvid0 => 3667,		name => 'l08u_brainlab' },
#	{ gvid0 => 3608,		name => 'l04u_labx18' },
#	{ gvid0 => 3500,		name => 'l03u_agr_underground' },
#	{ gvid0 => 3356,		name => 'l12_stancia_2' },
#	{ gvid0 => 3227,		name => 'l12_stancia' },
#	{ gvid0 => 3071,		name => 'l11_pripyat' },
#	{ gvid0 => 2816,		name => 'l10_radar' },
#	{ gvid0 => 2686,		name => 'l06_rostok' },
#	{ gvid0 => 2546,		name => 'l05_bar' },
#	{ gvid0 => 2499,		name => 'hospital' },
#	{ gvid0 => 2458,		name => 'limansk' },
#	{ gvid0 => 2445,		name => 'agroprom_underground' },
#	{ gvid0 => 2247,		name => 'military' },
#	{ gvid0 => 2146,		name => 'yantar' },
#	{ gvid0 => 1901,		name => 'marsh' },
#	{ gvid0 => 1805,		name => 'red_forest' },
#	{ gvid0 => 1650,		name => 'agroprom' },
#	{ gvid0 => 1432,		name => 'darkvalley' },
#	{ gvid0 => 1205,		name => 'garbage' },
#	{ gvid0 => 934,		name => 'escape' },
...
sub gvid_by_name {
	foreach my $info (levels_info) {
		if ((defined $info->{name}) and ($_[0] eq $info->{name})) {
			return $info->{gvid0};
		}
	}
	return undef;
...
sub read {
	...
	$cf->close();
	if ((::split_spawns()) && (not(::use_graph()))) {
		print "reading graph_points...\n";
		my $work_dir = getcwd();
		for my $level (@{$self->{levels}}) {
			my $gvid = (levels::gvid_by_name($level));
			my $sfn = ''.substr($work_dir, 0,-7).'/levels/'.$level.'/level.spawn';
			my $spawn_file = stkutils::chunked_file->new($sfn, 'r');
			$self->read_alife_level($spawn_file, $gvid);
			$spawn_file->close();
			chdir $work_dir;
		}
	}
	...
	while (1) {
		...
		if ($index == 0) {
			...
			while (1) {
				...
				my $level = levels::name_by_gvid($object->{cse_object}->{game_vertex_id}) or die "unknown_level";
				my $id = 0;
				for my $level_1 (@{$self->{levels}}) {
					if ((defined $level_1) and ($level_1 eq $level)) {
						$id++;
					}
				}
				if ($id == 0) {
...
sub read_alife_level {
	...
	while (1) {
		...
		if (::split_spawns()) {
			...
			my $level = (levels::name_by_gvid($gvid) or 'unknown');
...
sub read_graph {
	...
	if (not (::use_graph())) {
		...
		for my $vertex (@{$self->{graph_vertices}}) {
			...
			foreach my $obj (@{$self->{alife_objects}}) {
				if (($obj->{cse_object}->{section_name} eq 'level_changer') && ($obj->{cse_object}->{dest_game_vertex_id} == $object->{cse_object}->{game_vertex_id})) {
					$object->{cse_object}->{name} = $obj->{cse_object}->{dest_graph_point};
				}
			}
			if (!defined $object->{cse_object}->{name}) {
			...
			my $level = (levels::name_by_gvid($object->{cse_object}->{game_vertex_id}) or 'unknown');
			push @{$self->{$level}}, $object;
		}
	}
}
sub write_graph {
...
sub import_alife {
	...
	foreach my $fn (split /,/, ($if->value('alife', 'source_files') or die)) {
		...
		print "importing alife objects from level $fn...\n";
...
sub import_af_spawn_slots {
	my $self = shift;
	my ($if) = @_;
	print "importing artefact spawn places...\n";
...
sub export_af_spawn_slots {
	...
	binmode $bin_fh;
	print "exporting artefact spawn places data...\n";
...
sub export_way {
	...
	my $default_level = ::use_soc_levels() ? 'l01_escape' : 'zaton';
	foreach my $object (@{$self->{way_objects}}) {
	...
	foreach my $object (@{$self->{alife_objects}}) {
		...
		if (!defined $lif) {
			...
			if (::use_graph()) {
				...
				$dir = ''.substr($bdir, 0, -7).'/levels/'.$level.'';
			}
		...
		$object->alife_object::state_split_spawns($lif, ($id_of_levels{$level})++);
		chdir ($bdir); 
	}
...
sub split_ways {
	...
	foreach my $object (@{$self->{way_objects}}) {
		...
		if (!defined $info) {
			...
			chdir ($dir) or die "cannot change way path to levels/$level\n";
...
Usage: acdc_cop -d all.spawn [-o outdir] [-w] [-l]
       acdc_cop -c all.ltx [-f flag1[,...]] [-a level:point] [-o outfile]
END
}
...
if (defined $spawn_file) {
	...
	if (defined $split_spawns) {
		die "you can't do this on level.spawn!!!" unless (not defined $level_spawn);
		$spawn->split_spawn();
		print "done!\n";
	} else {
