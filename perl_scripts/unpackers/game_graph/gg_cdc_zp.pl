...
#modified version or ggtool.pl by bardak
#version x.5
#xiani
...
sub import {
	...
	foreach my $level (@{$self->{levels}}) {
		...
		foreach my $section (@{$if->{sections_list}}) {
			my $vertex = gg_vertex->new();
			$vertex->import($if, $section, $edges_offset, $level_points_offset);
			push @{$self->{vertices}}, $vertex;
		}
		$if->close();
	}
	...
	foreach my $level (@{$self->{levels}}) {
		...
		foreach my $section (@{$if->{sections_list}}) {
			...
			if ($i == 18)
			{
				print "x:target_level_id=" .$edge->{target_level_id}."\n";
			}
			
			if (($prev_vertex != -1) and ($prev_vertex->{level_vertex_id} == $edge->{parent_level_vertex_id}))
			{
				#cont inside same vertex edges
				$prev_vertex->{edge_count}++;
				$edge->{parent_game_vertex_id} =  $prev_vertex->{gvid};
			}
			else
			{
				#find new vertex
				...
				foreach my $vertex (@{$self->{vertices}}) {
					...
					{
						if ($vertex->{level_vertex_id} == $edge->{parent_level_vertex_id})
						{
							$edge->{parent_game_vertex_id} = $gvid;
							$vertex->{gvid} = $gvid;
							$prev_vertex = $vertex;
							if ($vertex->{edge_count} == 0)
							{
								$vertex->{edge_index} = $i;
	...
	foreach my $vertex (@{$self->{vertices}}) {
		for (my $i = 0; $i < $vertex->{edge_count}; $i++) {
			...
			if ($vertex->{level_id} != $vertex2->{level_id}) {
				...
				print $fh2 "target_level_id = " .$vertex2->{level_id}. "\n";
				...
				print $fh "target_level_id = " .$vertex2->{level_id}. "\n";
...
sub dump_levels_info {
	...
	foreach my $vertex (@{$self->{vertices}}) {
		...
		{
			$levels_info{$name} = ();
			$levels_info{$name}{first_gvid} = $vid;
			if (defined $levels_info{$last_name})
			{
				$levels_info{$last_name}{last_gvid} = ($vid-1);
			}
		}
		$last_name = $name;
		$vid++;
	...
	$levels_info{$last_name}{last_gvid} = ($vid-1);
	my $if = stkutils::ini_file->new($fn. ".levels_info.ltx", 'w') or die "cannot write ". $fn. ".levels_info.ltx\n";
	my $fh = $if->{fh};
	print $fh "[levels]\n";
	foreach my $level (@{$self->{levels}}) {
		print $fh $level->{level_name}. "\n";
	}
	print $fh "\n";
	foreach my $level (@{$self->{levels}}) {
		print $fh "[$level->{level_name}]\n";
		print $fh "first_gvid = " .$levels_info{$level->{level_name}}{first_gvid}. "\n";
		print $fh "last_gvid = " .$levels_info{$level->{level_name}}{last_gvid}. "\n";
		print $fh "\n";
	}
	$if->close();
	print "done\n";
...
sub check_vertexes {
	...
	my $level_id = -1;
	foreach my $vertex (@{$self->{vertices}}) {
		my $game_vertex_id2 = 0;
		foreach my $vertex2 (@{$self->{vertices}}) {
			if (($vertex->{level_vertex_id} == $vertex2->{level_vertex_id}) and ($game_vertex_id != $game_vertex_id2))
			{
				my $level = $level_by_id{$vertex->{level_id}};
				my $level2 = $level_by_id{$vertex2->{level_id}};
				print "found conflicting vertexes:\n";
				print "[1]:";
				print "level=".$$level->{level_name};
				print "gvid=" .$game_vertex_id.":lvid=".$vertex->{level_vertex_id};
				print "\n";
				print "[2]:";
				print "level=".$$level2->{level_name};
				print "gvid=" .$game_vertex_id2.":lvid=".$vertex2->{level_vertex_id};
				print "\n";
			}
			$game_vertex_id2++;
		}
...
print "'S.T.A.L.K.E.R. Call of Pripyat' game.graph compiler/decompiler x.5\n";

sub usage {
	return <<END
Usage: gg_cdc.pl -d game.graph [-o outdir] [-lp level_points_read_mode]
       gg_cdc.pl -c game.graph [-o binfile] [-lp level_points_read_mode]
       
level_points_read_mode:
    bin - binary mode, read and write level_points without parsing
END
}
...
my $level_points_use;
our $block_lp = 0;
our $bin_level_points = 0;
our $bin_cross_tables = 1;
our $xedges_write = 1;
...
GetOptions(
	...
	'c=s' => \$gg_ltx,
	'o=s' => \$out,
	'lp=s' => \$level_points_use,
) or die usage();

if (defined $level_points_use) {
	if ($level_points_use eq "bin")
	{
		$bin_level_points = 1;
	}
	if ($level_points_use eq "bin2")
	{
	  ## NOT WORKING YET
		$bin_level_points = 2;
	}
	if ($level_points_use eq "skip")
	{
		$block_lp = 1;
	}
}
...
if (defined $gg_file) {
	...
	$gg->dump_levels_info($gg_file);
	
	print "exporting... \n";
	$gg->export($gg_file or 'game.graph');
} elsif (defined $gg_ltx) {
	die "bad params\n" if defined $gg_file;
	my $gg = game_graph->new();
	print "importing... \n";
	$gg->import($gg_ltx);
	#print "exporting... \n";
	#$gg->export($out or 'game.graph.new');
	print "writing... \n";
	$gg->write($out or 'game.graph.new');
} else {
	die usage();
}
