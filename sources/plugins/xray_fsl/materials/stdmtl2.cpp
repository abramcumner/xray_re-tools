...
class StdMtl2ClassDesc:public ClassDesc2 {
	...
	const TCHAR *	ClassName() { return _T("xrFSLMaterial"); }
	SClass_ID		SuperClassID() { return MATERIAL_CLASS_ID; }
	Class_ID 		ClassID() { return xrFSLMaterial_CLASS_ID; }
	const TCHAR* 	Category() { return _T("");  }
	// JBW: new descriptor data accessors added.  Note that the 
	//      internal name is hardwired since it must not be localized.
	const TCHAR*	InternalName() { return _T("xrFSLMtl"); }	// returns fixed parsable name (scripter-visible name)
...
IAutoMParamDlg* StdMtl2::m_pxrFSLShadersDlg;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// xrFSL Shaders param block
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
class xrFSLShadersAccessor : public PBAccessor
{
public:
	void Set(PB2Value& v, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)    // set from v
	{
...
static xrFSLShadersAccessor xrshadersAccessor;
...
static void SetComboBoxItems(HWND hWnd, int nIDDlgItem, const std::vector<std::string>& vecItems)
{
	...
	// Åñëè ñïèñîê èì¸í - ïóñòîé, òî äîáàâèì çíà÷åíèå ïî óìîë÷àíèþ è âûáåðåì åãî
	if (vecItems.empty())
		ComboBox_AddString(hwndComboBox, "default");
	ComboBox_SelectString(hwndComboBox, 0, "default");
}

// xr shader rollout dialog proc
class xrFSLShaderDlgProc : public ParamMap2UserDlgProc 
{
public:
	INT_PTR DlgProc(TimeValue t, IParamMap2 *map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
...
static xrFSLShaderDlgProc xrshaderDlgProc;

// shader parameters
static ParamBlockDesc2 xr_shader_blk ( 
									  xrFSL_Shaders, _T("X-Ray Shaders"),  0, &stdmtl2CD, P_AUTO_CONSTRUCT + P_AUTO_UI, xrFSL_ref, 

										//rollout
										IDD_XR, _T("X-Ray"), 0, 0, &xrshaderDlgProc, 

										// params
										std2_shader_type, _T("shaderType"), TYPE_INT, 				0, 		IDS_JW_SHADERTYPE, 	
										p_default, 		3, 
										p_ui, 			TYPE_INTLISTBOX, IDC_SHADER, 0,
										p_accessor,		&xrshadersAccessor,
										end, 
										/*
										std2_wire, 		_T("wire"), 		TYPE_BOOL, 				0, 		IDS_DS_WIREFRAME, 	
										p_default, 		FALSE, 
...
void StdMtl2::Reset() 
{
	...
	static Class_ID defaultSamplerID( GetMarketDefaults()->GetClassID(
		MATERIAL_CLASS_ID, xrFSLMaterial_CLASS_ID, _T("sampler"),
...
// HEY!! this should all be done automatically in ParamBlock init...
	macroRecorder->Disable();  // don't want to see this parameter reset in macrorecorder
		...
		SetSamplingOn( GetMarketDefaults()->GetInt(MATERIAL_CLASS_ID,
			xrFSLMaterial_CLASS_ID, _T("samplerEnable"), 0) != 0);
...
ParamDlg* StdMtl2::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp) 
{
	...
	//------------------------------------
	m_pxrFSLShadersDlg = stdmtl2CD.CreateParamDlg(xrFSL_Shaders, hwMtlEdit, imp, this);
	//-------------------------------------
...
BOOL StdMtl2::SetDlgThing(ParamDlg* dlg)
{
	...
	if (dlg == pShaderDlg)
	{
		...
		assert (ClassID() == xrFSLMaterial_CLASS_ID);
...
{ 
	...
	switch (id)
	{
		...
		case xrFSL_Shaders: return m_ppbxrShaders;
	}
	return NULL; 
}
...
RefTargetHandle StdMtl2::GetReference(int i) {
	switch(i) {
		...
		case xrFSL_ref: return m_ppbxrShaders;
...
void StdMtl2::SetReference(int i, RefTargetHandle rtarg) {
	switch(i) {
		...
		case xrFSL_ref:	m_ppbxrShaders = (IParamBlock2*)rtarg;	return;
		default: assert(0);
	}
}
...
Class_ID StdMtl2::ClassID() { return xrFSLMaterial_CLASS_ID; }
